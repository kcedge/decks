...
public class CardContainer extends AdapterView<ListAdapter> {
    ...
    public int onCardNumbered;
    public boolean mAnimationStarted;
    //public boolean mFlung;
    ...
    private void init() {
        ...
        mFlingSlop = viewConfiguration.getScaledMinimumFlingVelocity() + 400;
    ...
    public CardModel getCurrentCard(){
        return (CardModel) mListAdapter.getItem(onCardNumbered);
    }


    public void skipButtonClickedOn() {

        if (getChildCount() != 0 && mAnimationStarted == false) {
            mTopCard = getChildAt(getChildCount()-1);


            {
                final View topCard = mTopCard;
                mTopCard.setLayerType(LAYER_TYPE_SOFTWARE, null);
                ...
                duration = Math.min(300, 300);

                mTopCard = getChildAt(getChildCount() - 2);

                if (onCardNumbered >= getAdapter().getCount()) {
                    onCardNumbered = getAdapter().getCount() - 1;
                }
                final CardModel cardModel = (CardModel) getAdapter().getItem(onCardNumbered);
                if (mTopCard != null)
                    mTopCard.setLayerType(LAYER_TYPE_HARDWARE, null);

                if (cardModel.getOnCardSkipedListener() != null) {

                    cardModel.getOnCardSkipedListener().onSkip(cardModel.getmCardId());
                ...
                topCard.animate()
                        ...
                        .x(targetX)
                        .y(targetY)
//                        .rotation(Math.copySign(45, 10))//velocityX
                        .setListener(new AnimatorListenerAdapter() {
                            ...
                            public void onAnimationEnd(Animator animation) {
                                ...
                                SimpleCardStackAdapter simpleAdp = (SimpleCardStackAdapter) mListAdapter;
                                simpleAdp.addToBottom();
                                setAdapter(simpleAdp);
//   mTopCard =
                                ensureFull();
                            }
        ...
        onCardNumbered = onCardNumbered + 1;
    }
        ...
        if (mAnimationStarted == false) {
            switch (event.getActionMasked()) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    x = event.getX(pointerIndex);
                    y = event.getY(pointerIndex);
                    float prevLastTouchX = mLastTouchX;
                    float prevLastTouchY = mLastTouchY;
                    mLastTouchX = x;
                    mLastTouchY = y;
                    if (!mDragging) {
                        return true;
                    ...
                    //Removes card from layout it
                    if (prevLastTouchX > 881 || prevLastTouchX < 189) {
                        ...
                        CardModel cardModel = (CardModel) getAdapter().getItem(onCardNumbered);
                        if (mTopCard != null)
                            mTopCard.setLayerType(LAYER_TYPE_HARDWARE, null);
                        ...
                        if (cardModel.getOnCardDimissedListener() != null) {
                            onCardNumbered++;
                            if (prevLastTouchX > 881) {
                                cardModel.getOnCardDimissedListener().onLike(cardModel.getmCardId());
                            } else {
                                cardModel.getOnCardDimissedListener().onDislike(cardModel.getmCardId());
                            }
                        }
//                        if (prevLastTouchX < 189) {
//                            targetX = -1000;
//                            targetY = targetY;
//                        }
                        topCard.animate()
...
//            if (cardModel.getOnCardDimissedListener() != null) {
//                if (targetX > 0) {
//                    cardModel.getOnCardDimissedListener().onDislike(cardModel.cardNumber);
//                } else {
//                    cardModel.getOnCardDimissedListener().onLike(cardModel.cardNumber);
//                }
//            }
//
//            topCard.animate()
